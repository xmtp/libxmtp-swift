...
public protocol FfiConversationProtocol: AnyObject, Sendable {
    ...
    func getLastReadTimes() throws  -> [Data: Int64]
...
open func getLastReadTimes()throws  -> [Data: Int64]  {
    return try  FfiConverterDictionaryDataInt64.lift(try rustCallWithError(FfiConverterTypeGenericError_lift) {
...
@_documentation(visibility: private)
#endif
fileprivate struct FfiConverterDictionaryDataInt64: FfiConverterRustBuffer {
    public static func write(_ value: [Data: Int64], into buf: inout [UInt8]) {
        let len = Int32(value.count)
        writeInt(&buf, len)
        for (key, value) in value {
            FfiConverterData.write(key, into: &buf)
    ...
    public static func read(from buf: inout (data: Data, offset: Data.Index)) throws -> [Data: Int64] {
        let len: Int32 = try readInt(&buf)
        var dict = [Data: Int64]()
        dict.reserveCapacity(Int(len))
        for _ in 0..<len {
...
private let initializationResult: InitializationResult = {
    ...
    if (uniffi_xmtpv3_checksum_method_fficonversation_get_last_read_times() != 34283) {
        return InitializationResult.apiChecksumMismatch
    }
